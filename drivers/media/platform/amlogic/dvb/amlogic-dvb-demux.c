...
{
	int b; u8 neq = 0;
	if (!dmx->filter[fid].used || !dmx->channel[dmx->filter[fid].chan_id].used)
		return false;
	for (b = 0; b < AML_FILTER_LEN; b++) {
		u8 xor = p[b] ^ dmx->filter[fid].value[b];
		if (xor & dmx->filter[fid].maskandmode[b])   return false;
		if (xor & dmx->filter[fid].maskandnotmode[b]) neq = 1;
	}
	if (dmx->filter[fid].neq && !neq) return false;
	return true;
}
...
static void section_notify(struct aml_dmx *dmx, int fid, const u8 *p)
{
	...
	if (cid < 0 || cid >= AML_CHANNEL_COUNT) return;
	feed = dmx->channel[cid].feed;
	if (feed && feed->cb.sec)
		feed->cb.sec((u8 *)p, slen, NULL, 0, dmx->filter[fid].filter, 0);
}
...
static void hardware_match_section(struct aml_dmx *dmx, u16 sec_num, u16 buf_num)
{
	...
	if (sec_num >= AML_FILTER_COUNT) return;
	p        = (const u8 *)dmx->sec_buf[buf_num].addr;
	cid      = dmx->filter[sec_num].chan_id;
	need_crc = 1;

	for (i = 0; i < AML_FILTER_COUNT; i++) {
		if (dmx->filter[i].chan_id != cid) continue;
		if (!sec_filter_match(dmx, i, p))  continue;
		if (need_crc) {
			...
			if (feed && feed->feed.sec.check_crc) {
				...
				if (dem->check_crc32(feed, p, slen)) return;
			}
			need_crc = 0;
		}
...
void aml_dmx_process_section(struct aml_dmx *dmx)
{
	...
	for (i = 0; i < AML_SEC_BUF_COUNT; i++) {
		...
		if (!(busy & BIT(i))) {
			...
			if (++dmx->sec_buf_watchdog_count[i] >= AML_SEC_WD_BUSY0_MAX) {
...
static int sec_buf_alloc(struct aml_dmx *dmx)
{
	...
	aml_write_reg(dvb, SEC_BUFF_01_START(dmx->id),
		(u32)((((grp_addr[0]-base)>>8)<<16)|((grp_addr[1]-base)>>8)));
	aml_write_reg(dvb, SEC_BUFF_23_START(dmx->id),
		(u32)((((grp_addr[2]-base)>>8)<<16)|((grp_addr[3]-base)>>8)));
	...
	dev_info(dvb->dev, "dmx%d: sec_buf OK %d bytes phys=0x%08llx base=0x%04llx\n",
		 dmx->id, dmx->sec_total_len,
		 (unsigned long long)dmx->sec_pages_map,
		 (unsigned long long)(dmx->sec_pages_map >> 16));
	dev_info(dvb->dev, "dmx%d: sec_buf grp0=0x%08llx grp1=0x%08llx grp2=0x%08llx grp3=0x%08llx\n",
		 dmx->id,
		 (unsigned long long)grp_addr[0], (unsigned long long)grp_addr[1],
		 (unsigned long long)grp_addr[2], (unsigned long long)grp_addr[3]);
	return 0;
}

static void sec_buf_free(struct aml_dmx *dmx)
{
	if (!dmx->sec_pages) return;
	dma_unmap_single(dmx->dvb->dev, dmx->sec_pages_map,
			 dmx->sec_total_len, DMA_FROM_DEVICE);
	free_pages(dmx->sec_pages, get_order(dmx->sec_total_len));
	dmx->sec_pages = 0;
	dmx->sec_pages_map = 0;
...
static u32 chan_target(struct aml_dmx *dmx, int cid)
{
	...
	if (dmx->channel[cid].type == DMX_TYPE_SEC) {
		...
		case DMX_PES_VIDEO:    type = FM_PKT_VIDEO;     break;
		case DMX_PES_AUDIO:    type = FM_PKT_AUDIO;     break;
		case DMX_PES_SUBTITLE:
		case DMX_PES_TELETEXT: type = FM_PKT_SECTION;   break;
		case DMX_PES_PCR:      type = FM_PKT_SCR;       break;
		default:               type = FM_PKT_RECORDER;  break;
		}
	}
	dmx->channel[cid].pkt_type = type;
	return ((u32)type << FM_PID_TYPE_SHIFT) | (dmx->channel[cid].pid & 0x1FFF);
}
...
static u32 filter_target_byte(struct aml_dmx *dmx, int fid, int byte_idx)
{
	...
	if (byte_idx == 0) {
		...
		if ((mode == 0xFF) && mask) {
			u8 t = mask & 0xF0;
			if (t) { mb1 = 0; adv |= t ^ 0xF0; }
			v |= (value & 0xF0) | adv;
			t = mask & 0x0F;
			if (t) { mb  = 0; adv |= t ^ 0x0F; }
			v |= (value & 0x0F) | adv;
		}
		target = (mb1 << 15) | (mb << 14) |
			 (0 << 13) |
		...
		target = FM_FBN_MASK | (0 << 13) | ((u32)cid << FM_FBN_CID_SHIFT);
...
static int dmx_set_filter_regs(struct aml_dmx *dmx, int fid)
{
	...
	dev_dbg(dmx->dvb->dev,
		...
		dmx->id, fid, dmx->filter[fid].used ? dmx->filter[fid].chan_id : -1,
		even_fid, odd_fid);
	return 0;
}
...
static int dmx_alloc_chan(struct aml_dmx *dmx, int type, int pes_type, u16 pid)
{
	...
	for (i = 0; i < AML_CHANNEL_COUNT; i++)
		if (!dmx->channel[i].used) { id = i; break; }
...
{
	if (cid < 0 || cid >= AML_CHANNEL_COUNT || !dmx->channel[cid].used) return;
	...
	aml_write_reg(dmx->dvb, DEMUX_CHAN_RECORD_EN(dmx->id), dmx->chan_record_bits);
	dmx_set_chan_regs(dmx, cid);
	dmx_enable(dmx);
}

static int dmx_alloc_filter(struct aml_dmx *dmx, int cid, struct dvb_demux_filter *df)
{
...
{
	if (fid < 0 || fid >= AML_FILTER_COUNT || !dmx->filter[fid].used) return;
...
static void dmx_enable(struct aml_dmx *dmx)
{
	...
	aml_write_reg(dvb, DEMUX_CONTROL(dmx->id),
		      ENABLE_FREE_CLK_FEC_DATA_VALID              |
		      ENABLE_FREE_CLK_STB_REG                     |
		      TS_RECORDER_SELECT                          |
	...
	if (dmx->sf_mode) {
		...
		aml_write_reg(dvb, DEMUX_MEM_REQ_EN(dmx->id), AML_MEM_REQ_EN_HW);
		dev_info(dvb->dev, "dmx%d: MEM_REQ=HW (SECTION_AHB=1)\n", dmx->id);
	}

	dev_info(dvb->dev, "dmx%d: dmx_enable CTRL=%08x MASK=%08x STB_EN=1 FEC=%08x END=%08x HIU=%08x\n",
		 dmx->id,
		 ENABLE_FREE_CLK_FEC_DATA_VALID | ENABLE_FREE_CLK_STB_REG |
		 (record ? TS_RECORDER_SELECT : 0u) | (ts_en ? TS_RECORDER_ENABLE : 0u) |
		 SECTION_END_WITH_TABLE_ID | KEEP_DUPLICATE_PACKAGE |
		 STB_DEMUX_ENABLE | (record ? VIDEO2_FOR_RECORDER_STREAM : 0u),
		 AML_DEMUX_INT_MASK,
		 ((u32)fec_sel << FEC_SEL_SHIFT) | fec_ctrl,
...
int aml_dvb_start_feed(struct dvb_demux_feed *feed)
{
	...
	if (feed->type == DMX_TYPE_SEC) {
		...
		cid = dmx_alloc_chan(dmx, DMX_TYPE_SEC, DMX_PES_OTHER, feed->pid);
		if (cid < 0) { ret = cid; goto err; }
		...
		if (!dmx->sec_pages) {
			...
			if (ret) { dmx_free_chan(dmx, cid); goto err; }
		}

	} else {
		/* DVR / TS path */
		is_dvr = (feed->pid == 0x1FFF || (feed->ts_type & TS_PACKET));
		cid = dmx_alloc_chan(dmx, DMX_TYPE_TS, feed->pes_type, feed->pid);
		if (cid < 0) { ret = cid; goto err; }
		dmx->channel[cid].feed = feed;
		feed->priv = (void *)(long)cid;
	...
	atomic_inc(&dvb->feed_count);
	dev_info(dvb->dev, "dmx%d: start_feed OK cid=%d CTRL=0x%08x MASK=0x%08x STATUS=0x%x\n",
		 dmx->id, cid,
		 ENABLE_FREE_CLK_FEC_DATA_VALID | ENABLE_FREE_CLK_STB_REG |
		 STB_DEMUX_ENABLE, AML_DEMUX_INT_MASK, 0);
	return 0;
err:
		...
		if (feed->filter) {
			...
			while (df) {
				if (df->hw_handle != (u16)-1) {
					dmx_free_filter(dmx, (int)df->hw_handle);
					df->hw_handle = (u16)-1;
				}
				df = df->next;
			}
		}
		if (cid >= 0) dmx_free_chan(dmx, cid);

	} else {
		bool is_dvr = (feed->pid == 0x1FFF || (feed->ts_type & TS_PACKET));
		if (is_dvr && cid >= 0) {
			dmx->chan_record_bits &= ~BIT(cid);
			aml_write_reg(dvb, DEMUX_CHAN_RECORD_EN(dmx->id),
				      dmx->chan_record_bits);
...
int aml_dmx_dvb_init(struct aml_dmx *dmx, struct dvb_adapter *adap)
{
	...
	dmx->demux.dmx.capabilities = DMX_TS_FILTERING |
	...
	if (ret) { dvb_dmx_release(&dmx->demux); return ret; }
	ret = dvb_net_init(adap, &dmx->dvbnet, &dmx->demux.dmx);
	if (ret) { dvb_dmxdev_release(&dmx->dmxdev); dvb_dmx_release(&dmx->demux); return ret; }
...
{
	aml_write_reg(dmx->dvb, DEMUX_INPUT_TIMEOUT(dmx->id), AML_TIMEOUT_DEFAULT);
	return 0;
}
...
int aml_dmx_hw_init(struct aml_dmx *dmx)
{
	...
	for (i = 0; i < AML_CHANNEL_COUNT; i++) dmx->channel[i].used = false;
	for (i = 0; i < AML_FILTER_COUNT;  i++) { dmx->filter[i].used = false; dmx->filter[i].chan_id = -1; }

	dmx->dvr_buf_size = SZ_4M;
	dmx->dvr_buf = dma_alloc_coherent(dev, dmx->dvr_buf_size,
					  &dmx->dvr_dma, GFP_KERNEL);
	if (!dmx->dvr_buf)
		dev_warn(dev, "dmx%d: DVR buf alloc failed\n", dmx->id);
	else
		dev_info(dev, "dmx%d: DVR buf OK phys=%pad\n", dmx->id, &dmx->dvr_dma);

	ret = aml_smallsec_init(dmx); if (ret) goto err;
	ret = aml_timeout_init(dmx);  if (ret) goto err_ss;

	dev_info(dev, "dmx%d: hw_init OK\n", dmx->id);
	return 0;
err_ss: aml_smallsec_release(dmx);
err:
	if (dmx->dvr_buf)
		dma_free_coherent(dev, dmx->dvr_buf_size, dmx->dvr_buf, dmx->dvr_dma);
	return ret;
}
...
void aml_dmx_hw_release(struct aml_dmx *dmx)
{
	...
	if (dmx->dvr_buf) {
		dma_free_coherent(dvb->dev, dmx->dvr_buf_size, dmx->dvr_buf, dmx->dvr_dma);
		dmx->dvr_buf = NULL;
	}
}
...
int aml_dmx_set_source(struct aml_dmx *dmx, unsigned int source)
{
	...
	if (source >= AML_TS_SRC_FRONTEND_TS0 &&
		...
		u32 fec_sel_val = (p < AML_MAX_TS_INPUT && dvb->ts[p].is_serial)
			? (6 - p) : p;
		u32 fec = (fec_sel_val << FEC_SEL_SHIFT) | (dvb->ts[p].fec_ctrl & 0xFFF);
		u32 stb = 0;
...
int  aml_dmx_hw_pid_alloc(struct aml_dmx *dmx, u16 pid) { return -ENOSYS; }
